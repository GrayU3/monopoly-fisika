...
.cell-position {
  ...
  box-shadow: 0px 0px 5px rgba(0, 0, 0, 0.7);  /* Adds a shadow effect */
}

#jailpositionholder .cell-position {
  top: -4px !important; /* Moves the icon up */
  position: relative;
}
...
@media (max-width: 768px) {
  ...
  .cell {
    width: 50px; /* Smaller cells for mobile */
    height: 50px;
  }
...
@media (max-width: 480px) {
  .cell {
    width: 40px; /* Even smaller cells for very small screens */
    height: 40px;
  }
...
/* Ensure the colspan="9" cell and its content do not disrupt layout */
/* Add this to position the parent cell relatively */
.board-center {
  position: relative; /* Ensure jail is positioned relative to this cell */
  width: auto;
  padding: 0;
  margin: 0;
  border: none;
...
#jail {
  position: absolute;
  top: 50px; /* Move jail upward (adjust as needed) */
...
/* Import Tailwind CSS if using it */
@tailwind base;
@tailwind components;
@tailwind utilities;
...
.space {
  ...
  justify-content: center;
  font-size: 14px;
  background: linear-gradient(180deg, #ffffff, #f2d99d);
  text-align: center;
  padding: 5px;
}
